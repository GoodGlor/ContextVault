...
class OpenAILLMProvider:
    ...
    def __init__(
        ...
        *,
        ...
        built from ``api_key`` (falling back to the ``openai_api_key`` setting,
        then the SDK's own ``OPENAI_API_KEY`` resolution). ``model`` and
        ``max_tokens`` default to the ``openai_model`` / ``llm_max_tokens``
        settings.
        """
